...
const User = require('./TTUser.js');
const Queue = require('./ttqueue.js');

const { formatMonthDay, formatTime, getJson } = require('./utils')
...
var cmbot = function(_options) {
	...
	this.RESULT_USER_NOT_FOUND = 0;
	this.RESULT_USER_MOVED = 1;
	this.RESULT_USER_NOT_IN_QUEUE = 2;
	this.RESULT_INVALID_POSITION = 3;
...
//	this.setStrings();
	this.session = {
		...
		triggers: {},
		tagLog: [],
		tagLogLimit: 20, // Maximum number of songs' tags to store in the tag log
		roomChanged: false,
		timers: {
			autodj: false,
		...
		currentSong: false,
		currentDJ: false,
		autoDJFreeSpots: 2, // how many free spots there must be before I will autodj
		snags: [],
		last_song: {
			votes: {
				up: [],
	...
	this.settings = Object.assign({
		...
		idleDJTimeout: false,
		setlimit: false, //this.options.set_limit,
		triggerLimit: {},
		triggerBan: {},
		timezones: {},
		triggers: {},
		...
		queue: [],
		phrases: {},
		snag_song_score_ignore_idle: false,
		bannedArtists: {},
		bannedSongs: {},
		mobileWhitelist: {},
		acl: {
...
cmbot.prototype.eventReady = function() {
	...
	this.bot.on('ready', function() {
		cmbot.bot.roomRegister(cmbot.options.bot.roomid, function() {
			cmbot.bot.setAsBot();
		});
...
cmbot.prototype.eventRoomChanged = function() {
	...
	this.bot.on('roomChanged', function(data) {
		if (cmbot.session.roomChanged)
			return false;
		cmbot.session.roomChanged = true;
//		log("room changed: ", data);
//		log("djs:", data.room.metadata.js);
//		log("data: ", data.room.metadata.current_song);

		// Process songlog and save the tags for the last 10 songs
//		log("songlog: ", data.room.metadata.songlog);
		cmbot.session.songlog = data.room.metadata.songlog;

		var songlog = data.room.metadata.songlog;
		if (cmbot.lastfm !== false) {
			var songs = [];
			cmbot.settings.oldTagLog = cmbot.settings.tagLog;
			cmbot.settings.tagLog = [];
			cmbot.saveSettings();
			for (var i = songlog.length - 1; i >= 0; i--) {
				var artist = songlog[i].metadata.artist;
				var song = songlog[i].metadata.song;
				songs.push([artist, song, songlog[i]['_id']]);
			}
			cmbot.getPastSongTags(songs);
		}
		cmbot.currentSong = data;
		cmbot.session.currentSong = data.room.metadata.current_song;
		cmbot.session.snags = [];
...
//		log("users: ", data.users);
		...
		for(var userid in data.users) {
			...
			if (typeof cmbot.users[user.userid] != 'object') {
				...
				var newUser = new User({userid: user.userid, name: user.name, mod: cmbot.mods[user.userid] == 1 || user.acl >= 1, laptop: user.laptop});
				if (user.acl == 1)
		...
		cmbot.q.getQueue().forEach(function(userid) {
		...
		if (cmbot.settings.idleDJTimeout !== false) {
			cmbot.session.djs.forEach(function(userid) {
...
cmbot.prototype.eventPM = function() {
	...
	this.bot.on('pmmed', function(data) {
		if (cmbot.users[data.senderid] == undefined) {
			if (typeof cmbot.settings.remotechat[data.senderid] == 'object') {
				cmbot.users[data.senderid] = new User({userid: data.senderid, name: cmbot.settings.remotechat[data.senderid].name, mod: cmbot.mods[data.senderid] == 1});
				cmbot.users[data.senderid].present = false;
			} else {
				cmbot.bot.pm("Sorry, but you must join my room to use /remotechat for the first time.", data.senderid);
				return false;
			}
		}
		...
		var remotechat = false;
		if (cmbot.options.remotechat === true) {
			if (typeof cmbot.settings.remotechat[userid] == 'object') {
				if (cmbot.settings.remotechat[userid].remotechat === true)
					remotechat = true;
			}
		}

		// If user is not present and the command is not undefined and theCommand.outofroom != true ignore the PM
		if (!cmbot.users[userid].present && theCommand != false) {
			if (theCommand.outofroom !== true) {
				return false;
			}
		}

		// If the user is not present and command is blank and remotechat is false, ignore the PM
		if (!cmbot.users[userid].present && remotechat === false && command == '') {
			return false;
		}
...
//				log("user: ", cmbot.users[userid]);
				if (!theCommand.modonly || cmbot.users[userid].mod || cmbot.options.master_userid.indexOf(userid) > -1) {
					var go = true;
					if (theCommand.acl === true)
						go = false; // Enforce ACL restrictions
					if (theCommand.acl === true && cmbot.settings.acl[command] != undefined) {
						go = false;
		...
		} else if (cmbot.settings.triggers[escape(command)] != undefined && (cmbot.users[userid].mod || cmbot.options.user_trigger_pm === true)) {
...
//		log("votelog: ", data.room.metadata.votelog);
		...
		var userid = 0;
		data.room.metadata.votelog[0].forEach(function(prop) {
			userid = prop;
			return false;
		});
		if (typeof cmbot.users[userid] == 'object') {
			cmbot.users[userid].lastInteraction = new Date().getTime();
			try {
				...
				if (cmbot.users[userid].djing && cmbot.settings.idleDJTimeout !== false) {
					log("(after vote) setting idle dj check for user " + cmbot.users[userid].name);
					cmbot.activateIdleDJCheck(cmbot.users[userid]);
				} else {
					log("Wanted to set idle dj check for user " + cmbot.users[userid].name + " but couldn't. dj'ing: " + cmbot.users[userid].djing + ", idlDJTimeout: " + cmbot.settings.idleDJTimeout);
				}
			} catch (e) {
		...
		var numUsers = 0;
		for(var userid in cmbot.users) {
			var user = cmbot.users[userid];
			if (user.present)
				numUsers++;
		}

		if (cmbot.session.loved !== true) {
			var now = new Date().getTime();
			var num_listeners = 0;
			if (cmbot.settings.snag_song_score_ignore_idle !== false) {
				for (userid in cmbot.users) {
					var user = cmbot.users[userid];
					if (user.present && (((now - user.lastInteraction) / 1000) < (60 * cmbot.settings.snag_song_score_ignore_idle)))
						num_listeners++;
				}

				// Calculate percent of bops
				var bops = data.room.metadata.upvotes - data.room.metadata.downvotes;
				var song_score = Math.round((((bops / num_listeners) * 100) / 2) + 50);
				if (song_score >= cmbot.settings.snag_song_score && bops >= cmbot.settings.snag_threshold) {
					log("Yoinking track");
					cmbot.yoinkTrack();
					cmbot.bot.snag();
					cmbot.session.loved = true;
...
cmbot.prototype.eventDisconnected = function() {
	var cmbot = this;
	this.bot.on('disconnected', function(error) {
		log("Disconnected: ", error);
		cmbot.bot.roomRegister(cmbot.options.bot.roomid);
		cmbot.doCustomEvents('disconnected');
	});
}
...
cmbot.prototype.eventNewSong = function() {
	...
	this.bot.on('newsong', function(data) {
		cmbot.session.last_song = {
			votes: {
				up: cmbot.session.votes.up,
				down: cmbot.session.votes.down
			},
			snags: cmbot.session.snags,
			media: cmbot.session.currentSong
		};
		...
		cmbot.session.snags = [];
		cmbot.session.currentDJ = data.room.metadata.current_song.djname;
		cmbot.session.votes = {
			up: [],
			down: []
		};
...
//		log("djs: %o", data.room.metadata.djs);
		cmbot.currentSong = data;
//		cmbot.session.djs = cmbot.currentSong.room.metadata.djs;
		var song = data.room.metadata.current_song;
		cmbot.session.currentSong = song;
		log("New song playing: " + song.metadata.song + " by " + song.metadata.artist + " dj'd by " + song.djname);
		var artist = song.metadata.artist;
		var songid = song['_id'];

		// Get the tags for this song and store them in the tag log (if the bot is not the one dj'ing)
		if (cmbot.lastfm !== false && data.room.metadata.current_dj != cmbot.options.bot.userid) {
			cmbot.lastfm.getTags({
				...
				callback: function(result) {
					if (result.success) {
						...
						if (tags.length > 0) {
							if (cmbot.settings.tagLog.length >= cmbot.session.tagLogLimit - 1)
								cmbot.settings.tagLog.shift();
							cmbot.settings.tagLog.push({
								songid: songid,
								tags: tags
							});
							cmbot.saveSettings();
							log("Added tags to taglog");
						}
					} else {
						log("Error! (event new song)");
					}
				}
			});
		}
		var banned = false;
		if (!cmbot.options.bypass_bans) {
...
//				log("scrobble at: " + scrobbleAt);
		...
		// If the bot is the next dj, select a new song to play
		log("Data: ", data.room.metadata.djs);
		var djs = data.room.metadata.djs;
		for (var i = 0; i < djs.length; i++) {
			if (djs[i] == data.djid && djs[i + 1] == cmbot.options.bot.userid) {
				cmbot.selectNextSong();
			}
		}
		...
		try {
			if (cmbot.currentSong !== false) {
				...
				if (cmbot.options.mysql.enabled) {
					...
					cmbot.mysql.query('USE ' + cmbot.options.mysql.database);
					if (mysql !== false) {
									...
									if (err) {
										...
										mysql.query("INSERT INTO song VALUES(?, ?, ?, ?, ?, ?, ?, ?)",
												[
													...
													song.metadata.album ? song.metadata.album.replace('/\\/', '') : '',
...
//						cmbot.activateCasinoRollTimer();
					...
					if(cmbot.session.warned) { // User got a warning to skip the song, so don't increase their playcount, and send them a thanks.
						clearTimeout(user.timers.warning);
						cmbot.bot.pm("Thanks for skipping your song. Your playcount won't increase for this play.", userid);
					} else {
						user.playcount++;
					}
					...
					if (((user.playcount >= cmbot.settings.setlimit && cmbot.session.enforcement) || user.escortme)) {
						...
						if(data.room.metadata.current_dj == cmbot.options.bot.userid && data.room.metadata.djcount <= 2) {
							// Do nothing, don't escort the bot
							log("2 or less dj's on deck, so I will stay up.");
						} else {
							user.escortme = false;
							log(user.name + " has hit song limit of " + cmbot.settings.setlimit + ", removing from the decks.");
							cmbot.bot.remDj(user.userid, function(data) {
								log("Result removing " + user.name + ": ", data);
							});
							cmbot.users[user.userid].djing = false;
						}
					} else if (user.idleDJEscort && cmbot.session.enforcement) {
						// User's song was playing when they reached their idle limit, so give them a warning and remove them from the decks in 1 minute unless they chat/vote (the timer gets reset in those events)
						user.idleDJEscort = false;
...
//			log("User laptop: " + newDj.laptop);
			if (!cmbot.options.allow_mobile_djs && !newDj.mod && (newDj.laptop == 'iphone' || newDj.laptop == 'android') && cmbot.settings.mobileWhitelist[newDj.userid] == undefined) {
				...
				if (numEscorts >= 3) {
					cmbot.bot.speak("Hasta la vista, meatbag!", function(result) {
...
//											bot.pm("http://goo.gl/krnve", newDj.userid);
											cmbot.bot.speak("Hasta la vista, meatbag!", function(result) {
												cmbot.bot.speak("http://i.imgur.com/rwzT5.gif", function(result) {
...
cmbot.prototype.autodj = function() {
	...
	if (cmbot.session.djing)
		...
		log("AutoDJ is off.");
	else if (freeSpots < cmbot.settings.autoDJFreeSpots)
...
//		delete currentSong.room.metadata.djs[data.user[0].userid];
		log("Removing " + user.userid + " from cmbot.session.djs");
...
cmbot.prototype.eventSnagged = function() {
	...
	cmbot.bot.on('snagged', function(data) {
		log("song snagged: ", data);
		cmbot.session.snags.push(data.userid);
		log("snags: ", cmbot.session.snags);
...
cmbot.prototype.eventRegistered = function() {
	...
	this.bot.on('registered', function(data) {
		...
		if (typeof cmbot.users[userid] != 'object') {
			user = new User({userid: userid, name: data.user[0].name, mod: cmbot.mods[userid] == 1 || data.user[0].acl > 0, laptop: data.user[0].laptop});
			...
			cmbot.users[userid] = user;
		}
		else {
			cmbot.users[userid].present = true;
			cmbot.users[userid].lastInteraction = now();
			cmbot.users[userid].laptop = data.user[0].laptop;
			cmbot.users[userid].name = data.user[0].name;
		...
		if (cmbot.settings.shitlist[userid] != null) {
			...
			// First, see if the user has changed their name since being shitlisted, and update that name in the shitlist entry for the user (so they can be unshitlisted)
			if(cmbot.users[userid].name != cmbot.settings.shitlist[userid].name) {
				cmbot.settings.shitlist[userid].name = cmbot.users[userid].name;
				cmbot.saveSettings();
			}
...
cmbot.prototype.speakOrPM = function(text, pm, userid, callback) {
	if (pm)
		this.bot.pm(text, userid, callback);
	else
		this.bot.speak(text, callback);
};
...
cmbot.prototype.doCustomEvents = function(event, data) {
	if (typeof this.customEvents[event] == 'object' && (data.userid != this.options.bot.userid || event == 'pmmed')) {
...
cmbot.prototype.initOptions = function(_options) {
	...
	[_options.settings_file, _options.dj_file].forEach(function(file) {
...
cmbot.prototype.setupMessages = function() {
	...
	if (isNaN(this.options.message_interval)) {
		...
		this.options.messages.forEach(function(message, index) {
			var message = cmbot.options.messages[index].replace('%setlimit%', cmbot.settings.setlimit);
...
cmbot.prototype.modpm = function(text, modsToPm, from, fromBot) {
	...
	if (modsToPm === false) {
		modsToPm = [];
		for(var each_userid in cmbot.mods) {
			var remotechat = false;
			if (typeof cmbot.settings.remotechat[each_userid] == 'object')
				if (cmbot.settings.remotechat[each_userid].remotechat === true)
					remotechat = true;
			if (cmbot.users[each_userid] != undefined &&
					(cmbot.users[each_userid].present || remotechat) &&
...
cmbot.prototype.getPlayCounts = function() {
	var cmbot = this;
	var counts = [];
	// Refresh the list of dj's based on currentSong (which is updated each time a new song is playing). This will have the most up to date list of
	// which dj's are on the decks. Needed because occasionally the bot's list of dj's on decks becomes incorrect for some reason.
//	cmbot.session.djs = cmbot.currentSong.room.metadata.djs;
	this.session.djs.forEach(function(userid) {
		try {
			var user = cmbot.users[userid];
			if (user.djing) {
				counts.push(user.name + ": " + user.playcount);
...
cmbot.prototype.getMysqlClient = function() {
	log("Getting mysql client.");
	var cmbot = this;
	if(false !== cmbot.mysql) {
		try {
			log("Ending mysql connection.");
			cmbot.mysql.end();
		} catch(e) {
			log("Exception ending mysql: ", e);
		}
	}
//	if (cmbot.mysql === false) {
		try {
			var mysql = require('mysql');
			cmbot.mysql = mysql.createConnection({
			...
			log("Exception getting mysql client: ", e);
		}
//	}
//	return mysql;
	return cmbot.mysql;
...
cmbot.prototype.checkQueue = function() {
	...
	if (this.session.djs.length < cmbot.session.max_djs) {
		if (this.q.getQueueLength() > 0 && this.session.enforcement) {
			var foundUser = false;
			this.q.getQueue().forEach(function(userid) {
				var user = cmbot.users[userid];
				if (user != undefined) {
					if (!foundUser) {
						if (user.userid == cmbot.options.bot.userid) {
							...
							if (!user.afk && user.present && !user.djing && user.timers.queueTimer === false) {
								...
								user.timers.queueTimer = setTimeout(function() {
									...
									cmbot.checkQueue();
									cmbot.bot.pm("You missed your chance to get on deck, so you've been marked AFK. You still have your spot in the queue, but you'll be skipped until you type /back.", user.userid);
								}, 60000 * 3);
								foundUser = true;
							} else if (user.djing) {
...
cmbot.prototype.loadModule = function(module) {
	...
	if (customModule.customCommands !== undefined) {
		...
		for (var i = 0; i < customCommands.length; i++) {
			...
			if (typeof customCommand.name != 'string' || customCommand.name == '')
				...
				if(typeof customCommand.onUnload == 'function') {
					customCommand.onUnload(this);
				}
				this.customCommands = Object.assign(this.customCommands, comm);
...
cmbot.prototype.commands = {
	...
	'setlimit': {
		command: function(options) {
			...
			var speak = false;
			if (options.arg == '') {
				text = "The set limit (number of tracks per DJ) is " + (options.cmbot.settings.setlimit == 0 ? "unlimited" : options.cmbot.settings.setlimit) + ".";
				speak = options.pm ? false : true;
			} else if (options.arg.match(/^[0-9]+$/)) {
				if(!user.mod) {
					options.cmbot.speakOrPM("Sorry, but only mods may change the set limit.", options.pm, options.userid);
				} else {
					options.cmbot.settings.setlimit = parseInt(options.arg);
					options.cmbot.saveSettings();
					speak = true;
					text = "The set limit (number of tracks per DJ) is now " + (parseInt(options.arg) == 0 ? "unlimited" : options.arg) + ".";
				}
			} else
		...
		modonly: false,
		pmonly: false,
		help: 'Change the set limit (number of songs each DJ can play). Set to 0 for unlimited.'
	},
	'bansong': {
		command: function(options) {
			var text = "";
	...
	'autodjlimit': {
		command: function(options) {
			var num_spots = parseInt(options.arg, 10);
			if('' === options.arg) {
				options.cmbot.bot.pm("I won't autodj until there are " + options.cmbot.settings.autoDJFreeSpots + " free spots.", options.userid);
			} else if(false === Number.isInteger(num_spots)) {
				options.cmbot.bot.pm("Please specify a valid number.", options.userid);
			} else if (num_spots > options.cmbot.session.max_djs) {
				options.cmbot.bot.pm("Cannot set the limit to a number higher than the number of allowed DJ's.", options.userid);
			} else {
				options.cmbot.settings.autoDJFreeSpots = num_spots;
				options.cmbot.saveSettings();
				options.cmbot.bot.pm("I won't autodj until there are " + num_spots + " free spots.", options.userid);
			}
		},
		modonly: true,
		pmonly: true,
		help: 'Display or set the number of free DJ spots there needs to be before I will autodj.'
	},
	'modpm': {
		command: function(options) {
			var text = "";
	...
	'modchat': {
		...
		modonly: true,
		pmonly: true,
		outofroom: true, // Enable this command even if the user is not in the room.
		help: 'Enable or disable modchat for yourself. Usage: /modchat [on|off]'
	},
	'remotechat': {
		command: function(options) {
			var text = "";
			if (options.cmbot.settings.remotechat[options.userid] == undefined) {
				options.cmbot.settings.remotechat[options.userid] = {
					remotechat: false
				};
			}
			if (options.cmbot.options.remotechat === false) {
				text = "Sorry, but remote mod chat is disabled.";
			} else if (options.arg == '') {
				var status = (options.cmbot.settings.remotechat[options.userid].remotechat === true ? "on" : "off");
				text = "You have remotechat " + status + ".";
			} else if (options.arg == "on") {
				if (options.cmbot.users[options.userid] == undefined)
					text = "Sorry, you'll have to join the room I'm in before you can use this command for the first time";
				else {
					options.cmbot.settings.remotechat[options.userid] = {
						remotechat: true,
					...
					options.cmbot.saveSettings();
					text = "You will now see modchat even when you are not in the room.";
				}
			} else if (options.arg == "off") {
				options.cmbot.settings.remotechat[options.userid].remotechat = false;
				options.cmbot.saveSettings();
				text = "You will no longer see modchat when you are not in the room.";
			} else {
				text = "Usage: /remotechat [on|off]";
			}
			options.cmbot.bot.pm(text, options.userid);
		},
		modonly: true,
		pmonly: true,
		outofroom: true, // Enable this command even if the user is not in the room.
		help: 'Turn remote modchat on or off. Usage: /remotechat [on|off]'
	},
	'loadmodule': {
		command: function(options) {
			log("Loading module " + options.arg + ".");
			try {
	...
	'setdjtimeout': {
		command: function(options) {
			if (options.arg != parseInt(options.arg)) {
				...
				options.cmbot.saveSettings();
				options.cmbot.session.djs.forEach(function(userid) {
...
//				try {
				var text = '';
				res.list.forEach(function(song, i) {
...
//				}
	...
	'modtrigger': {
		command: function(options) {
			...
			if (options.arg.match(/^([^ ]+) (.*)$/)) {
				...
				function createModTrigger(trigger, saying) {
					var text = '';
					var success = false;
					...
					log("got to end, success = " + success);
					if (success)
				...
				if (urls.length == 0) {
					createModTrigger(trigger, saying);
				} else {
					log("urls: ", urls);
	...
	'trigger': {
		command: function(options) {
			...
			if (options.arg.match(/^([^ ]+) (.*)$/)) {
				...
				if (preexisting) {
					options.cmbot.bot.pm("This trigger already exists. Please use /retrigger to update it.", options.userid);
				} else {
					options.cmbot.createTrigger(options);
				}
			}
		},
		modonly: true,
	...
	'retrigger': {
		command: function(options) {
			if (options.arg.match(/^([^ ]+) (.*)$/)) {
				...
				var preexisting = options.cmbot.settings.triggers[trigger] != undefined;
				if (!preexisting) {
					options.cmbot.bot.pm("This trigger doesn't exist. Please use /trigger to create it.", options.userid);
				} else {
					options.cmbot.createTrigger(options);
				}
			}
		...
		modonly: true,
		pmonly: true,
		help: 'Update a trigger. Same usage as /trigger.'
	},
	'shitlist': {
		command: function(options) {
			if (options.arg == '') {
			...
			var arg = options.arg;
			for (var userid in options.cmbot.users) {
				var thisUser = options.cmbot.users[userid];
	...
	'unshitlist': {
		command: function(options) {
			...
			var found = false;
			for(var each_userid in options.cmbot.settings.shitlist) {
				var obj = options.cmbot.settings.shitlist[each_userid];
...
//						log("got tags: ", result);
						if (result.success) {
							var tags = [];
							result.tags.forEach(function(tag_obj) {
					...
					callback: function(result) {
						if (result.success) {
							...
							options.cmbot.shortenUrl('http://www.last.fm/user/' + options.cmbot.lastfm.username + '/library/music/' + artist, function(result) {
								var url = 'http://www.last.fm/user/' + options.cmbot.lastfm.username + '/library/music/' + artist.replace(" ", "+");
								if (result.success) {
									url = result.url;
								}
								text += ' ' + url;
...
//					log("setting count: " + arg);
			...
			if (!options.cmbot.isFFA()) {
				if (!options.cmbot.users[options.userid].mod && !options.cmbot.options.allow_mobile_djs && !options.cmbot.options.allow_mobile_djs && (options.cmbot.users[options.userid].laptop == 'iphone' || options.cmbot.users[options.userid].laptop == 'android') && options.cmbot.settings.mobileWhitelist[options.userid] == undefined) {
					...
					if (!result.success) {
						...
						if (options.cmbot.options.quiet_add === true) {
							if (options.pm) {
								options.cmbot.bot.pm(result.queue, options.userid);
							} else {
								options.cmbot.bot.speak(options.cmbot.users[options.userid].name + " is now #" + result.spot + " in the queue.");
							}
						} else {
	...
	'deckshitlist': {
		command: function(options) {
			...
			if (options.arg == "")
				...
				var reason;
				log("arg = " + options.arg);
				for(var user in options.cmbot.users) {
					var thisUser = options.cmbot.users[user];
			...
			if (options.arg.length <= 3) {
				...
				options.cmbot.bot.playlistAll(function(res) {
					try {
						...
						log("Exception searching playlist: ", e);
					}
				});
			}
		},
	...
	'removesong': {
		...
		pmonly: true,
		acl: true,
		help: 'Remove a song from my playlist. Usage: /removesong <integer> where <integer> is the index of the song on my playlist. To find this for a particular song, use /searchplaylist. It will be the number before the colon. e.g. /removesong 0 (will remove the first song on my playlist).'
	},
	'shortenurl': {
		command: function(options) {
			...
			if (options.cmbot.session.nosong) {
				...
				if (options.cmbot.session.warned) {
					...
					if (!warnUser.djing) {
						...
						warnUser.timers.warning = setTimeout(function() {
							options.cmbot.session.warned = false;
							options.cmbot.bot.remDj(warnUser.userid);
							options.cmbot.bot.pm("Sorry, you didn't skip in time.", warnUser.userid);
						}, 15 * 1000);
		...
		command: function(options) {
			if (options.cmbot.isFFA()) {
				...
				if (options.arg.match(/^(.*)\s([0-9]+)$/)) {
					...
					options.cmbot.moveUserInQueue(userToMove, position, options.userid, function(result) {
						log("result = " + result);
						if (result == options.cmbot.RESULT_USER_NOT_FOUND) {
							options.cmbot.bot.pm("User not found.", options.userid);
						} else if (result == options.cmbot.RESULT_USER_MOVED) {
							options.cmbot.bot.pm(userToMove.name + " moved to position " + (position), options.userid);
						} else if (result == options.cmbot.RESULT_USER_NOT_IN_QUEUE) {
							options.cmbot.bot.pm(userToMove.name + " is not in the queue!", options.userid);
						} else if (result == options.cmbot.RESULT_INVALID_POSITION) {
							options.cmbot.bot.pm("Invalid position.", options.userid);
						}
					});
				} else {
	...
	'dj': {
		command: function(options) {
			...
			if (options.cmbot.isFFA()) {
				...
				var qlength = 0;
				options.cmbot.q.getQueue().forEach(function(userid) {
...
//			log("counts: ", counts);
	...
	'ban': {
		command: function(options) {
			var found = false;
			var foundArtist = '';
			for (var key in options.cmbot.settings.bannedArtists) {
	...
	'enforcement': {
		...
		modonly: true,
		pmonly: false,
		help: 'Turn queue enforcement on or off. If it\'s off, I won\'t escort a DJ off the deck if they get on the deck when it isn\'t their turn.'
	},
	'refresh': {
		command: function(options) {
			var text = '';
	...
	'front': {
		command: function(options) {
			...
			if (user === false) {
				...
				if (options.cmbot.q.queueArray.indexOf(options.userid) == -1) {
					options.cmbot.q.newAddUser(user);
				}
				options.cmbot.moveUserInQueue(user, 1, options.userid, function(result) {
					options.cmbot.bot.pm(user.name + " added to the front of the queue.", options.userid);
				});
			}
		},
		modonly: true,
		pmonly: true,
		help: 'Add a user to the front of the queue, or move them there if they are already in the queue.'
	},
	/*
	 'votes': {
	 command: function(options) {
...
//				return false;
		...
		getJson(httpoptions.url)
			.then(artist_result => {
				...
				if (artist_result.resultsPage.totalEntries === 0) {
					...
					return getJson(`http://api.songkick.com/api/3.0/artists/${artist_id}/calendar.json?apikey=${options.cmbot.options.songkick.api_key}`)
						.then(result => {
							if (result.resultsPage.totalEntries == 0) {
								...
								var shows = [];
								result.resultsPage.results.event.forEach(function(event, index) {
									if (index <= 6)
										shows.push(event.venue.metroArea.displayName + ' ' + formatMonthDay(new Date(event.start.date)));
								});
								log("shows: ", shows);
	...
	'bannedartists': {
		command: function(options) {
			var a = [];
			for (var artist in options.cmbot.settings.bannedArtists) {
	...
	'triggerban': {
		command: function(options) {
			...
			if (user == false) {
				...
				options.cmbot.settings.triggerBan[user.userid] = now() + (60 * 60 * 24 * 1000); // this is when this ban expires
	...
	'kick': {
		command: function(options) {
			var text;
			if (options.arg == '') {
				text = "Please specify a user to kick!";
			}
			var result = false;
			var mytext = '';
			var user = false;
			...
			if(user === false) {
				...
				options.cmbot.bot.bootUser(user.userid, reason !== false ? reason : '');
				text = user.name + " has been kicked by " + options.cmbot.users[options.userid].name + ".";
				mytext = user.name + " has been kicked.";
				result = true;
			}
//			options.cmbot.bot.pm(text, options.userid);
			if (result) {
	...
	'help': {
		command: function(options) {
			...
			if (options.arg != '') {
				...
				var commands = [];
				var allCommands = Object.assign({}, options.cmbot.commands, options.cmbot.customCommands);
				for(var commandName in allCommands) {
					var command = allCommands[commandName];
				// [Object.assign({}, options.cmbot.commands, options.cmbot.customCommands), function(commandName, command) {
...
cmbot.prototype.getPastSongTags = function(previous_songs) {
	var cmbot = this;
	//cmbot.settings.tagLog = [];
//	log("songs: ", previous_songs);
	if (previous_songs.length > 0 && cmbot.settings.tagLog.length < cmbot.session.tagLogLimit) {
		var song = previous_songs.shift();
//		log("song: ", song);

		var songid = song[2];
		var gottags = false;
		for (var i = 0; i < cmbot.settings.oldTagLog.length; i++) {
			if (songid == cmbot.settings.oldTagLog[i].songid) {
				log("Already have tags for " + song[1] + " by " + song[0]);
				cmbot.settings.tagLog.push({
					songid: songid,
					tags: cmbot.settings.oldTagLog[i].tags
				});
				gottags = true;
			}
		}
		if (gottags) {
			cmbot.getPastSongTags(previous_songs);
		} else {
			log("Getting tags for " + song[1] + " by " + song[0]);
			cmbot.lastfm.getTags({
				artist: song[0],
				track: song[1],
				callback: function(result) {
...
//					});
		...
		log("Done getting tags: ", cmbot.settings.tagLog);
		cmbot.saveSettings();
	}
}
...
//		log("Before sort, rolls: ", cmbot.session.casino_data.rolls);
		var rolls = [];
		for (var userid in cmbot.session.casino_data.rolls)
			rolls.push([userid, cmbot.session.casino_data.rolls[userid]]);
//		log("Rolls: ", rolls);
...
cmbot.prototype.matchSongTags = function(songs, callback) {
	var cmbot = this;
	// First, find the first song that has no tags
	var idx = false;
	for (var i = 0; i < songs.length; i++) {
		if (typeof songs[i].tags == 'undefined') {
			idx = i;
			break;
		}
	}
	...
	if (idx === false) {
		// All songs have tags, so return
		if (typeof callback == 'function') {
			callback(songs);
		}
	} else {
		log("Getting tags for " + songs[idx].title + " by " + songs[idx].artist);
		cmbot.lastfm.getTags({
			artist: songs[idx].artist,
			track: songs[idx].title,
			callback: function(result) {
				log("tags result: ", result);
				var tags = [];
				...
				songs[idx].tags = tags;
				cmbot.matchSongTags(songs, callback);
			}
		});
	}
};

cmbot.prototype.createTrigger = function(options) {
	var trigger = RegExp.$1.toLowerCase();
	var saying = RegExp.$2;
	var results = /shorten:([^ ]+)/g.execAll(saying);
	var urls = [];
...
cmbot.prototype.moveUserInQueue = function(userToMove, position, userid, callback) {
	var cmbot = this;
	var result = false;
	if (userToMove === false) {
		result = cmbot.RESULT_USER_NOT_FOUND;
	}
	else if (position <= cmbot.q.getQueueLength() && position > 0) {
		var queue = cmbot.q.getQueue();
		var oldPosition = -1;
		queue.forEach(function(userid, index) {
			var user = cmbot.users[userid];
		...
		if (oldPosition > -1) {
			...
			cmbot.q.moveUser(oldPosition, position);
			result = cmbot.RESULT_USER_MOVED;
		} else {
			result = cmbot.RESULT_USER_NOT_IN_QUEUE;
		}
	}
	else {
		result = cmbot.RESULT_INVALID_POSITION;
	}
	if (typeof callback == 'function') {
		callback(result);
	}
...
cmbot.prototype.log = log;

cmbot.prototype.selectNextSong = function() {
	var cmbot = this;
	log("Executing selectNextSong");
	if (cmbot.settings.tagLog.length == 0) {
		cmbot.bot.pm("No tags in the tag log!", options.userid);
	} else {
		...
		cmbot.bot.playlistAll(function(res) {
			...
			var songs = [];
			for (var i = 0; i < 10; i++) {
				var min = plLength <= 20 ? 0 : 20;
				log("min: ", min);
				var idx = getRandomNumber(min, plLength);
				log("idx: ", idx);
				log("pllength: ", plLength);
				var randomSong = res.list[idx];
				log("random song: ", randomSong);
				songs.push({
					idx: idx,
					artist: randomSong.metadata.artist,
					title: randomSong.metadata.song
				});
			}
			log("Calling matchSongTags with ", songs);
			cmbot.matchSongTags(songs, function(songsWithTags) {
				log("Got songs with tags: ", songsWithTags);

				// Go through the tag log and compile a count of each tag
				var tags = {};
				for (var i = 0; i < cmbot.settings.tagLog.length; i++) {
					var song = cmbot.settings.tagLog[i].tags;
					for (var a = 0; a < song.length; a++) {
						if (tags[song[a]] == undefined)
							tags[song[a]] = 0;
						tags[song[a]]++;
					}
				}
				...
				// Grab the top 5 tags
				for (var a = 0; a < 5; a++) {
					tags.push(tuples[a][0]);
				}

				// Now find the song that shares the most tags
				for (var a = 0; a < songsWithTags.length; a++) {
					songsWithTags[a].matches = 0;
					for (var b = 0; b < tags.length; b++) {
						if (songsWithTags[a].tags.indexOf(tags[b]) > -1)
							songsWithTags[a].matches++;
					}
				}
				var topSong = false;
				for (var a = 0; a < songsWithTags.length; a++) {
					if (topSong === false || songsWithTags[a].matches > topSong.matches)
						topSong = songsWithTags[a];
				}
				if (topSong === false) {
					topSong = songsWithTags[getRandomNumber(0, songsWithTags.length)];
				}
				cmbot.bot.playlistReorder(topSong.idx, 0, function(result) {
					if (result.success) {
						log("Top song chosen.");
					}
				});
